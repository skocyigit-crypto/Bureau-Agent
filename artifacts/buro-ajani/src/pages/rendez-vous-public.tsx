...
export default function RendezVousPublicPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && data.offer) {
        ...
        setError(data.error || t("rendezVousPublic.error.offerNotFound"));
      }
    } catch {
      setError(t("rendezVousPublic.error.loadOffer"));
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  const fetchLiveSlots = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && Array.isArray(data.slots)) {
        ...
        setSlotsError(data.error || t("rendezVousPublic.error.loadSlots"));
        setLiveSlots([]);
      }
    } catch {
      setSlotsError(t("rendezVousPublic.error.networkSlots"));
      setLiveSlots([]);
    } finally {
  ...
  async function selectSlot(index: number) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setError(data.error || t("rendezVousPublic.error.selectFailed"));
        if (data.code === "conflict" || data.code === "already") {
          await load();
        }
      }
    } catch {
      setError(t("rendezVousPublic.error.networkConfirm"));
    } finally {
      setSubmitting(null);
    }
  }
  ...
  async function rescheduleSlot(slotIndex: number, slot: PublicSlot) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setError(data.error || t("rendezVousPublic.error.rescheduleFailed"));
      ...
      setError(t("rendezVousPublic.error.networkReschedule"));
    } finally {
      setSubmitting(null);
    }
  }
  ...
  async function cancelAppointment() {
    ...
    if (!(await confirmAction({ title: t("rendezVousPublic.confirmCancelTitle"), description: t("rendezVousPublic.confirmCancelDesc"), confirmLabel: t("rendezVousPublic.confirmCancelLabel"), destructive: true }))) return;
    setCancelling(true);
    setError("");
    try {
      ...
      if (res.ok && data.success) {
        ...
        setError(data.error || t("rendezVousPublic.error.cancelFailed"));
        await load();
      }
    } catch {
      setError(t("rendezVousPublic.error.networkCancel"));
    } finally {
      setCancelling(false);
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0f1729] via-[#1a2744] to-[#0f1729] p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle>{offer?.orgName || t("rendezVousPublic.fallbackTitle")}</CardTitle>
          {offer && !loading && !error && (
            <CardDescription>
              {isCancelled
                ? t("rendezVousPublic.descCancelled")
                : isConfirmed
                  ? view === "reschedule"
                    ? t("rendezVousPublic.descReschedule")
                    : t("rendezVousPublic.descConfirmed")
                  : isExpired
                    ? t("rendezVousPublic.descExpired")
                    : t("rendezVousPublic.descChoose", { reason: offer.reason })}
            </CardDescription>
          )}
        </CardHeader>
        <CardContent className="space-y-4">
          {loading && (
            <div className="flex items-center justify-center py-8 text-muted-foreground">
              <Loader2 className="mr-2 h-5 w-5 animate-spin" /> {t("rendezVousPublic.loading")}
            </div>
          )}
          ...
          {!loading && offer && isCancelled && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="font-medium">{t("rendezVousPublic.cancelledTitle")}</p>
              <p className="text-xs text-muted-foreground">
                {t("rendezVousPublic.cancelledText", { org: offer.orgName })}
              </p>
            </div>
          )}
          ...
          {!loading && offer && isConfirmed && view === "summary" && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <p className="font-medium">{t("rendezVousPublic.confirmedTitle")}</p>
              {selectedLabel && <p className="text-sm text-muted-foreground">{selectedLabel}</p>}
              <p className="text-xs text-muted-foreground">{t("rendezVousPublic.confirmedThanks")}</p>
              ...
              <div className="mt-2 flex w-full flex-col gap-2">
                ...
                >
                  <CalendarClock className="mr-2 h-4 w-4" /> {t("rendezVousPublic.chooseOther")}
                </Button>
                <Button
                  variant="ghost"
                ...
                >
                  ...
                  {t("rendezVousPublic.cancelAppointment")}
                </Button>
              </div>
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              {loadingSlots && (
                <div className="flex items-center justify-center py-6 text-muted-foreground">
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" /> {t("rendezVousPublic.loadingSlots")}
                </div>
              )}
              ...
              {!loadingSlots && slotsError && (
                <div className="flex flex-col items-center gap-2 py-4 text-center">
                  ...
                  <Button variant="outline" size="sm" onClick={fetchLiveSlots}>
                    <RefreshCw className="mr-2 h-3 w-3" /> {t("rendezVousPublic.retry")}
                  </Button>
                </div>
              )}
              ...
              {!loadingSlots && liveSlots !== null && liveSlots.length === 0 && !slotsError && (
                <div className="py-4 text-center">
                  <p className="text-sm text-muted-foreground">
                    {t("rendezVousPublic.noSlots", { org: offer.orgName })}
                  </p>
                </div>
              )}
              ...
              >
                {t("rendezVousPublic.back")}
              </Button>
              <p className="pt-1 text-center text-xs text-muted-foreground">
                {t("rendezVousPublic.durationTz", { min: offer.durationMinutes, tz: offer.timezone })}
              </p>
            </div>
          )}
          ...
          {!loading && offer && isExpired && (
            <div className="flex flex-col items-center gap-2 py-6 text-center">
              <XCircle className="h-8 w-8 text-destructive" />
              <p className="text-sm text-muted-foreground">
                {t("rendezVousPublic.expiredText", { org: offer.orgName })}
              </p>
            </div>
          )}
