...
import { FileText, Search, Plus, Loader2, Trash2, Edit, MoreHorizontal } from "lucide-react";
...
export default function DevisPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) { const d = await res.json(); setRows(d.devis || []); setTotal(d.total || 0); }
  ...
  useEffect(() => {
    ...
    const cn = sp.get("clientName");
    if (cn) {
      setForm(f => ({ ...f, clientName: cn }));
      setEditingId(null);
      setDialogOpen(true);
    }
  ...
  const openEdit = async (d: Devis) => {
    setEditingId(d.id);
    setForm({ ...EMPTY_FORM, reference: d.reference, title: d.title, clientName: d.clientName, status: d.status });
    setDialogOpen(true);
    try {
      const res = await fetch(`${BASE}/api/devis/${d.id}`, { credentials: "include" });
      if (!res.ok) return;
  ...
  return (
    <div className="container mx-auto px-4 py-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><FileText className="w-6 h-6 text-blue-500" />Devis</h1>
          <p className="text-sm text-muted-foreground">Gérez vos propositions commerciales.</p>
        </div>
          ...
          {rows.map(d => (
            <div key={d.id} className="flex items-center gap-3 px-4 py-3 hover:bg-muted/20">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{[d.clientName, d.clientCompany].filter(Boolean).join(" · ")}</p>
              </div>
              <span className="text-sm font-semibold">{fmtMoney(d.totalAmount, d.currency)}</span>
      ...
      <div className="text-xs text-muted-foreground">
        <button onClick={() => setLocation("/factures-client")} className="underline">Voir les factures →</button>
      </div>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs">Référence</Label><Input value={form.reference} onChange={e => setForm(f => ({ ...f, reference: e.target.value }))} placeholder="DEV-2026-001" /></div>
              <div>
            ...
            <div>
              ...
              <GhostTextarea
                fieldType="quote_comment"
                context={{ title: `${form.reference || "Devis"} — ${form.title}`, contactName: [form.clientName, form.clientCompany].filter(Boolean).join(" · ") }}
                value={form.notes}
                onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                rows={3}
                placeholder="Remerciements, validité de l'offre, modalités..."
              />
            </div>
            <div>
