...
const STATUT_CONFIG: Record<string, { label: string; variant: string }> = {
  ...
  annulee: { label: "Annulee", variant: "outline" },
};

const TYPE_LABELS: Record<string, string> = {
  acompte: "Acompte", intermediaire: "Intermediaire", finale: "Finale", avoir: "Avoir",
};
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-950/30 dark:to-emerald-900/10">
          <CardContent className="p-4">
            <div className="text-sm text-emerald-600 dark:text-emerald-400 font-medium">Encaisse</div>
                  ...
                  {factures.map(f => {
                    ...
                    return (
                      <tr key={f.id} className="border-b hover:bg-muted/50 transition-colors">
                        ...
                        <td className="py-3 px-2 hidden md:table-cell"><Badge variant="outline">{TYPE_LABELS[f.type] || f.type}</Badge></td>
                        ...
                        <td className="py-3 px-2"><Badge variant={cfg.variant as any}>{cfg.label}</Badge></td>
                        <td className="py-3 px-2 hidden lg:table-cell text-muted-foreground">{format(new Date(f.createdAt), "dd/MM/yyyy", { locale: fr })}</td>
                        <td className="py-3 px-2 text-right">
                          <div className="flex items-center justify-end gap-1">
                            {f.statut === "en_attente" && (
                              <Button variant="ghost" size="icon" onClick={() => handleMarquerPayee(f.id)} title="Marquer payee"><CreditCard className="w-4 h-4 text-emerald-500" /></Button>
                            )}
                            <Button variant="ghost" size="icon" onClick={() => handleDelete(f.id)} title="Supprimer"><Trash2 className="w-4 h-4 text-destructive" /></Button>
                          </div>
                        </td>
                      </tr>
                    );
