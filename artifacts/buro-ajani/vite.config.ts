...
function applySecurityHeaders(res: { setHeader(k: string, v: string): void }) {
  ...
  // En dev, l'apercu Replit charge l'iframe depuis un autre origin, donc
  // CORP same-origin doit etre relache (cross-origin) sinon le navigateur
  // refuse de servir le HTML a l'iframe parent. En prod, on garde same-origin.
