...
// file runs as part of `pnpm --filter @workspace/db push`; this function
// runs on every api-server boot so dev/staging/prod cannot drift.
//
// We use drizzle's sql`` template (no ${} interpolations, no user input)
// instead of sql.raw() so the repo stays consistent with the convention
// described in helpers/accent-search.ts. The PL/pgSQL `$func$ … $func$`
// dollar-quoting is passed through verbatim because it does not contain
// ${} brace pairs that drizzle would parse.
