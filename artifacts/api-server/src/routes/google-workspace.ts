...
router.get("/google-workspace/hub", async (req, res): Promise<void> => {
  try {
    ...
    const apps = GOOGLE_APPS.map(app => ({
      ...app,
      connected: app.scopes.some(s => grantedScopes.includes(s)),
      connectionStatus: connMap.get(app.id)?.status || "deconnecte",
    ...
    res.json({
      ...
      tokenValid: tokens.length > 0 && tokens[0].expiresAt && tokens[0].expiresAt > new Date(),
      apps,
