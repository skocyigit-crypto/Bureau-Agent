...
- [Live API e2e from scripts](live-api-e2e-from-scripts.md) — non-browser POSTs need Origin header (CSRF 403 else); use login wantsToken bearer not __Host- cookie; logged-in users can't be hard-deleted (audit append-only) → anonymize.
