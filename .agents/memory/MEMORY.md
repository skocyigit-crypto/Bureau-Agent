...
- [DB pool fan-out + retry](db-pool-fanout-retry.md) — per-org count fan-out under concurrent crons exhausts the pool; collapse to count(*) FILTER and wrap idempotent reads in withDbRetry (transient conn errors only); don't bump pool first.
...
- [Per-user AI learning layer](ai-user-learning-layer.md) — agent learns per-employee (ai_user_profile_facts) not just per-org; buildLearnedContextBlock cache keyed `org:user`, invalidation MUST prefix-purge `org:*`; pass acting userId to personalize a call site.
- [Webhook manual retry single-executor](webhook-manual-retry.md) — user "replay" must re-queue for the single background worker, never also send from HTTP; resetting failed→pending trips the worker's stale-pending net → duplicate delivery.
- [load() self-dep refetch loop](effect-load-refetch-loop.md) — a fetch useCallback in useEffect(()=>load(),[load]) must not dep on state it mutates (cached/arr.length); use refs + reqGen, else silent continuous refetch.
- [Detail-page route-id fetch race](detail-page-fetch-race.md) — raw fetch-by-route-id in useEffect needs a captured-vs-current id guard before each setState; React Query hooks are already id-keyed/race-free.
- [Assistant tool enum descriptions](assistant-tool-enum-descriptions.md) — enum values in LLM-tool descriptions feed eq() filters/inserts; keep canonical or model silently matches 0 rows; enum-validate create_* fields.
- [Domain status enums (tasks/calls)](task-status-enum.md) — tasks=en_attente/en_cours/termine/annule, calls=repondu/manque/messagerie (French); wrong/English literals fail silently (0 rows), typecheck won't catch; English only in API param mapping layer.
