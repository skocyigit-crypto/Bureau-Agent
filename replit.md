...
- **AI Integration:** Leverages 3 AI providers (Gemini, OpenAI, Anthropic) via Replit AI Integrations for various functionalities:
    ...
    - **Intelligence Centrale:** `POST /ai/central-intelligence` - Proactive AI briefing system that reduces mental load by 80%. Scans all data (tasks, messages, calls, contacts, stock, checkins) in parallel, computes health score, and produces: (1) Urgency management with CRITIQUE/A_PLANIFIER/INFO categorization, (2) Auto-drafted responses for unread messages, (3) Smart contact follow-up list (15+ days since last call), (4) Stock anomaly alerts cross-referenced with tasks, (5) Daily actionable advice. Frontend: `CentralIntelligence` component on dashboard with score gauge, tabbed interface (Actions/Reponses/Relances/Stock), copy-to-clipboard for draft replies, and 15min session caching.
