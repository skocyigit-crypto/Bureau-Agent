...
- **Organisation Management (Licence Admin):** Super Admin-only page at `/organisations` (sidebar: "Lisans") with full CRUD for client organisations. Create organisation with auto-generated licence key + subscription + admin user. Change plans, edit details, activate/deactivate, delete. **Complete onboarding flow:** When creating an org, admin user fields (prenom, nom, email) are included; a secure 12-char password is auto-generated (upper+lower+digits+special), the admin user is created in the same DB transaction, and a single professional HTML email is sent containing licence key + admin login credentials + app access link. **Email notification** via nodemailer (SMTP config: `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`). **Password management:** "Reinitialiser MDP" button per org card regenerates admin password and sends updated credentials. "Envoyer la licence" resends licence-only email. **User credential endpoints:** `POST /api/auth/users/:id/send-credentials` generates new password + emails it; `POST /api/auth/users/create-and-send` creates user with auto-generated password + emails it. Users page dropdown has "Envoyer mot de passe" per user. Add user dialog supports auto-generate + email mode (leave password blank). Email templates: `sendLicenseEmail` (licence + optional credentials), `sendCredentialsEmail` (user credentials), `sendWelcomeEmail` (self-registration). Resend licence endpoint: `POST /api/organisations/:id/resend-license` (supports `{resetPassword: true}` to also reset admin password). API at `/api/organisations/*`. Each organisation gets a unique licence key (format: `ADB-XXX-HEXHEX`). Stats dashboard shows total/active/trial/paid counts. Routes: `artifacts/api-server/src/routes/organisations.ts`, `artifacts/api-server/src/routes/auth.ts`. UI: `artifacts/buro-ajani/src/pages/organisations.tsx`, `artifacts/buro-ajani/src/pages/users.tsx`. Email service: `artifacts/api-server/src/services/email.ts`.
- **Mobile App (Expo):** Full-featured companion mobile app with complete feature parity. Screens: Dashboard (quick actions grid, recent calls, performance stats), Calls (CRUD, search, filter, detail modal, tap-to-call), Contacts (CRUD, category filter, detail with call/email actions), Tasks (CRUD, search, filter, checkbox toggle, priority indicators), Messages (CRUD, type filters, mark-as-read), Calendar (month navigation, event list, create events, contact info), Stock (CRUD, search, status badges, detail view), Analytics (stats, progress bars, response rate), Settings (profile, subscription info, app info), AI Agents (run agents, view scores/reports, super agent synthesis), Automations (rules with toggle, execution logs, stats), Check-ins/Pointage (live timer, bureau/distance/terrain modes, pause/resume, history), Users (CRUD, role management, send credentials, activate/deactivate), Audit Log (security log with action/resource filters, stats), Integrations (software catalog with category filters, connection status), Organisations (super admin: create org with admin, license management, password reset). Reusable components: `FAB.tsx` (floating action button), `FormModal.tsx` (bottom-sheet form), `DetailModal.tsx` (detail view with actions). All screens have pull-to-refresh. "Plus" tab grouped: Communication, Outils, Intelligence Artificielle, Administration (role-gated), Compte. Admin screens visible only to admin/super_admin roles; Organisations visible only to super_admin. Routes: `/(tabs)/index`, `/(tabs)/calls`, `/(tabs)/contacts`, `/(tabs)/tasks`, `/(tabs)/more`, `/messages`, `/calendar`, `/stock`, `/analytics`, `/settings`, `/ai-agents`, `/automations`, `/checkins`, `/users`, `/audit-log`, `/integrations`, `/organisations`.
