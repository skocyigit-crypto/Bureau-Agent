...
- **Real Photo Banners:** Professional AI-generated photography integrated as banner cards across key pages. Each banner uses a gradient overlay (matching the page's color theme) with white text. Images are in `assets/images/`: `office-team.png` (dashboard), `call-center.png` (calls), `reception-desk.png` (contacts), `analytics-work.png` (analytics), `security-server.png` (settings/security tab). All use `loading="lazy"` and `decoding="async"` for performance.
