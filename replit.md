...
- **License Management & Billing Dashboard:** Admin-only `/gestion-licence` page with comprehensive license security, payment tracking, and invoice management. Features: subscription overview with plan limits, security alerts (trial expiry, unpaid invoices, overdue client invoices, missing bank info), client invoice management with email sending and payment reminder system (escalating levels 1-4 with urgency-styled emails), automatic overdue payment reminder batch processing (7-day cooldown), auto-invoice generation with usage-based overage calculation, billing settings management (IBAN/BIC/SIRET/TVA/automation toggles), and full audit logging. 6-tab interface: Vue d'ensemble, Factures clients, Paiements, Rappels, Parametres, Journal. DB tables: `payment_reminders`, `license_audit_log`. API: `/api/license-management/*` (dashboard, send-payment-reminder, auto-generate-invoice, send-invoice-email, auto-reminders, audit-log, update-billing-settings).
...
- **Face Recognition System (Mobile + API):** AI-powered face recognition for office security. Mobile screen (Expo) with camera access, face scanning, profile registration (with role assignment: visiteur/employe/client/fournisseur/vip), contact linking, recognition history, and stats dashboard. Backend API with multi-AI analysis (Gemini/OpenAI fallback), mood detection, personalized greetings, security level assessment (normal/attention/alerte), and suggested actions. Cross-tenant ID ownership validation, input sanitization, and safe error messages. DB: `face_profiles`, `face_recognition_logs`. API: `/api/face/*` (profiles, register, recognize, logs, stats, search-contact). Mobile: `/face-recognition` screen with 4 tabs (Scanner, Enregistrer, Profils, Historique). Files: `face-recognition.ts`, `face-recognition.tsx`.
- **AI Commandant Engine (20 Capabilities):** Comprehensive AI orchestration engine with multi-provider fallback (Gemini → OpenAI → Anthropic). 17 API endpoints covering: smart call response with contact history, call compilation with auto-task/appointment creation, auto-create tasks/appointments from any interaction, email smart reply with tone adaptation, email compilation/categorization, overdue task/invoice reminders with email sending, meeting compilation with auto-task/reminder creation, photo+GPS location with reverse geocoding (OpenStreetMap + Google Maps link), employee statistics with AI performance analysis, payment/invoice overview with financial health scoring, Google Drive file sending, attachment saving, daily AI briefing with business weather metaphor, task reminder emails, AI-powered cross-module smart search (contacts/tasks/events/invoices/prospects with AI summary), and AI text analysis (6 modes: sentiment/summary/entities/action items/translate/rewrite). Frontend: `/commandant-ia` 10-tab dashboard (Briefing with Smart Search + AI Text Tools, Telephone, Email, Reunions, Taches, Finance, Photo+GPS, Drive, Rappels, Stats). API: `/api/commandant/*`. Files: `ai-commandant.ts`, `commandant-ia.tsx`.
