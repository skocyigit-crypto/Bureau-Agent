...
- **AI Assistant ULTRA (28 Executable Actions):** The AI chat assistant has full operational control with real-time access to all data (calls, contacts, tasks, messages, stock, calendar, projects, prospects). Executable actions include: CRUD for tasks/contacts/events/projects/prospects, email sending (Resend), calendar scheduling, follow-up planning, prospect-to-client conversion, web search (via Gemini), report generation, global data search, data export, bulk operations (escalate/complete), stock updates, and notifications. All actions are org-scoped for multi-tenant security.
