...
- **Email Invitation System:** Admin-initiated user invitations with 72-hour expiry, single-use cryptographic tokens, license limit enforcement (pending+active vs maxUsers), role assignment. Professional HTML email via Gmail/Resend. Public invitation accept page at `/invitation/:token` (no auth required). API: send/resend/cancel/verify/accept invitations. DB: `invitations` table with org isolation.
...
- **User Tracking (created_by/updated_by):** All core entities (contacts, tasks, calls, messages, checkins, calendar events) track which user created and last modified each record. API routes set `createdBy`/`updatedBy` from session on INSERT/UPDATE and resolve user display names (`"Prenom N."` format) via `enrichWithUserNames`/`enrichSingle` helpers in `artifacts/api-server/src/helpers/user-tracking.ts`. Frontend detail views (contact detail, call detail, task edit dialog) display "Cree par" / "Modifie par" with timestamps.
- **Core Features:** Dashboard, Call, Contact, Task, Message Management, AI-driven automation, Command Palette, Notifications Center, and team performance analytics. *(Factures, Stock, CRM/Prospects, Devis, Comptes Clients, Projets modules have been removed — handled by separate external software.)*
...
- **Voice Command System ("Hey Bureau") — AI-Powered:** Full voice assistant with wake word detection for both web and mobile. API endpoint at `/api/voice/command` (POST) with AI-powered intent parsing via Gemini (with regex fallback). 15 intents: daily briefing, count calls/tasks/contacts, recent calls, urgent tasks, create task, call contact, search, calendar, performance, greeting, time, thanks, and help. Unknown commands get natural AI conversational responses via Gemini. Web floating widget (bottom-left, VoiceAssistant.tsx) uses Web Speech API for speech-to-text and SpeechSynthesis for text-to-speech, with conversation history. Mobile screen (`voice-assistant.tsx`) with chat-style interface, tap-to-speak, and command quick list. Route mapping uses French-slug routes (`/appels`, `/taches`, `/calendrier`, `/analyse`).
