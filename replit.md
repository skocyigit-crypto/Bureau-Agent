...
- **AI Integration:** Leverages 3 AI providers (Gemini, OpenAI, Anthropic) via Replit AI Integrations for various functionalities:
    ...
    - **Intelligence Centrale / Assistant Executif:** `POST /ai/central-intelligence` - High-performance executive assistant with 3 modules: (1) MODULE RECONNAISSANCE IA: Crisis management with "Mode Commando" when score < 90, structured resolutions as [PROBLEME] -> [SOLUTION] -> [ACTION], per-module categorization (reconnaissance/communication/logistique); (2) MODULE COMMUNICATION: Eisenhower matrix (urgent_important/urgent/important/info) for messages, 90%-finalized draft replies, post-send action suggestions, fiches de rappel with call scripts for missed calls; (3) MODULE LOGISTIQUE: Stock flux tendu with 48h rupture anticipation, pre-filled bon de commande, planning optimization with productivity gap analysis. 16 parallel DB queries including upcoming 48h tasks cross-referenced with stock, recent missed calls with contact details. Frontend: `CentralIntelligence` component with commando mode visual (red gradient, pulsing siren), score gauge with severity indicator, 4-tab interface (Resolutions/Reponses/Appels/Logistique), Valider/Copier/Traiter action buttons on every item, strategic directive at bottom. Session cached 15min. PII-minimized, link-whitelisted, category-validated response sanitization.
