...
/**
 * Extrait les declarations `pgTable`, en suivant les accolades.
 *
 * Premiere version: une expression reguliere qui fermait le corps sur le
 * premier `\n}` — c'est-a-dire une accolade en colonne zero. Elle marchait pour
 * `pgTable("x", {` ... `\n}` et ratait silencieusement la forme ou l'appel est
 * reparti sur plusieurs lignes, dont l'accolade fermante est indentee:
 *
 ...
 *   );
 *
 * Deux tables passaient ainsi a la trappe. Le garde-fou les a vues comme
 * absentes du schema, donc comme des tables a supprimer, et a bloque le build —
 * ce qu'il devait faire: son propre appel en CI existe pour reperer une derive
 * de SA lecture, et il a rempli exactement ce role. Une expression reguliere
 * qui suppose une mise en forme n'est pas une lecture de code; compter les
 * accolades, si.
 */
export function declaredSchema(dir = schemaDir) {
  ...
  for (const file of fs.readdirSync(dir)) {
    ...
    const nameRe = /pgTable\(\s*["'`]([\w_]+)["'`]\s*,/g;
    let m;
    while ((m = nameRe.exec(src))) {
      const tableName = m[1];
      const open = src.indexOf("{", nameRe.lastIndex);
      if (open === -1) continue;
      let depth = 0, close = -1;
